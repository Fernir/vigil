...
test.describe("Site Management", () => {
  ...
  test("should edit site", async ({ page }) => {
    ...
    await fillInput(
      ...
      "Test Site for Edit Updated Name",
    );
    await page.click('button:has-text("Save")');
    await page.waitForURL("/");
    await expect(
      page.locator("text=Test Site for Edit Updated Name"),
    ).toBeVisible();
  ...
  test("should delete site", async ({ page }) => {
    ...
    await page.click(
      'button[aria-label="Delete site"], button:has-text("Delete")',
    );
    await page.waitForURL("/");
    await waitForHydrate(page);
    await expect(
      page.locator("text=Test Site for Delete").first(),
    ).not.toBeVisible();
  });
